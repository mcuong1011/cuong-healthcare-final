          ...
          {loading ? (
            ...
            <div>
              <div className="mb-3 text-sm text-gray-600">
                ...
                {formatDoctorName(
                  doctors.find((d) => d.id == form.doctor) || {}
                )}{" "}
                on {form.date}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto">
                {availableSlotsData.slots.map((slot, index) => (
                  <button
                    ...
                    className={`p-4 rounded-lg border text-sm font-medium transition-all duration-200 text-left ${getSlotStatusColor(
                      slot
                    )}`}
                  >
              ...
              {form.selectedSlot && (
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <div className="text-blue-700">
                    {formatTimeRange(
                      form.selectedSlot.start_time,
                      form.selectedSlot.end_time
                    )}
                  </div>
                </div>
              )}
